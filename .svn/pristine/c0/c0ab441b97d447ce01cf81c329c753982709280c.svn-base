...
<div class="row">
	<div class="col-xs-12">
		<div class="btn-group" style="display:none">
			<button id="createTaskBtn" type="button" class="btn btn-default btn-info btn-sm btn-round" data-toggle="modal" style="display:none">
				<i class="fa fa-plus-square"></i>
				新建
			</button>
			<button type="button" onclick="goViews()" class="btn btn-info btn-blue btn-sm btn-round" style="display:none">
				<i class="fa fa-eye"></i>
				查看
			</button>
			<!-- 
...
$('.page-content-area').ace_ajax('loadScripts', scripts, function() {
  jQuery(function($) {
	...
	var g=$("a[href$='${ctx}/office/notice']");
	...
	/*
		   		{name:'createdateStr',index:'createdateStr',width:150,editable: true,search:false},,'操作',
		   		{name:'myac',index:'', width:80, fixed:true, sortable:false, resize:false,search:false,formatter:actionFormatter1,editable: false}
	*/

	$(grid_selector).jqGrid({
		 url:'${ctx}/office/notice/queryViewList',
		 datatype: 'JSON',
		 colNames:['ID','通知标题', '发布时间','是否浏览'],
		 colModel:[
		   		...
		   		{name:'createdate',index:'createdate',width:50,editable: true,search:false,formatter:createdateFormatter},
                {name:'readflag',index:'readflag',width:50,editable: true,stype: "select",formatter:readflagFormatter,
		   			searchoptions:{  
                        sopt:["eq"],
                        stype:"select",
                        dataUrl:"${ctx}/office/notice/readflagSearch"
                    }
                }
		 ],
		 jsonReader : {
...
//编辑选定记录
function showRow(id){
	$.get("${ctx}/office/notice/viewNotice?id="+id+"&"+Math.random(1000) , '', function(data){ 
		var modal = $('#taskModal');
		modal.html(data);
