...
<div class="row">
	<div class="col-xs-12">
		<div class="btn-group">
			<button id="showDownBtn" type="button" class="btn btn-info btn-blue btn-sm btn-round" data-toggle="modal">
				<i class="fa fa-eye"></i>   
				查看
			</button>
		</div>
...
$('.page-content-area').ace_ajax('loadScripts', scripts, function() {
  jQuery(function($) {
	...
	$(grid_selector).jqGrid({
		 url:'${ctx}/office/task/taskDownlist',
		 datatype: 'JSON',
		 colNames:['ID','任务编号', '任务名称','任务期数','发布时间','上报时间','上报频率','操作'],
		 colModel:[
		   		...
		   		{name:'beginTime',index:'beginTime',width:120,editable: true},		
		   		{name:'endTime',index:'endTime',width:120,editable: true},
		   		{name:'taskHz',index:'taskHz',width:120,editable: true},
...
$(window).triggerHandler('resize.jqGrid');//trigger window resize to make the grid get the correct size
	...
	*/
	...
	$("#showDownBtn").click(function(e){
		var rowId = $(grid_selector).jqGrid('getGridParam','selrow');
		if(rowId){
			window.location.href="${ctx}/index#page/office/task/taskDownReport/"+rowId;
		}else{
...
function showRow(id){
	window.location.href="${ctx}/index#page/office/task/taskDownReport/"+id;
}
