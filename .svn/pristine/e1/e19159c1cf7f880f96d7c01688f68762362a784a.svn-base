	  ...
	  $.subscribe('exportItem', function(event,data) {
			 ...
			 if (selectedIds!=null && selectedIds.length>0){
				...
				if(filters!="undefined" && filters!=null){
						$.fileDownload('<s:url value="/item/exportQuery.action"/>', { httpMethod : "POST", data: {filters : filters,pageSize:10}});
				}else{
					   $.fileDownload('<s:url value="/item/exportQuery.action"/>', { httpMethod : "POST", data: {pageSize:10}});
				 }
		     }
	  });
});
...
<div class="app-panel">
	<div class="title">当前位置：采购计划 &gt;&gt; 项目查询 &gt;&gt; 所有项目</div>
	<div class="toolbar">
...
<div id="itemWrapper">
	...
	<s:url var="listQuery" action="listQuery" namespace="/item" />
	<sjg:grid dataType="json" href="%{listQuery}" pager="true"
		rownumbers="true" multiselect="true" multiboxonly="true"
		navigator="true" altRows="true" navigatorSearch="true"
