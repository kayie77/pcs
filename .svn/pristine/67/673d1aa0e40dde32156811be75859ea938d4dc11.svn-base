...
<div class="row">
	<div class="col-xs-12">
		...
		<button id="createDivisionBtn" type="button" class="btn btn-default btn-info btn-sm btn-round" data-toggle="modal">
			<i class="fa fa-plus-square"></i>选择区域
		</button>
...
//编辑选定记录
function showRow(id){
	$.get("${ctx}/office/message/viewMessage?id="+id+"&"+Math.random(1000) , '', function(data){ 
		var modal = $('#taskModal');
		modal.html(data);
...
$('.page-content-area').ace_ajax('loadScripts', scripts, function() {
  jQuery(function($) {
	...
	$(grid_selector).jqGrid({
		 url:'${ctx}/office/message/querySendedMessage',
		 datatype: 'JSON',
		 colNames:['ID','状态','','','','接收人','主题', '发布时间'],
		 colModel:[
		   		{name:'id',index:'id', width:100, hidden:true,editable: true},
		   		{name:'readflag',index:'readflag', width:30, editable: true, sortable:false,formatter:statusFormatter,stype: "select",
		   			searchoptions:{  
                        sopt:["eq"],
                        stype:"select",
		   		...
		   		{name:'important',index:'important', width:100, hidden:true,editable: true},
		   		{name:'needreplay',index:'needreplay', width:100, hidden:true,editable: true},
		   		{name:'replayflag',index:'replayflag', width:100, hidden:true,editable: true},
...
//新建记录
function createRow(){
	$.get("${ctx}/office/message/edit?"+Math.random(1000) , '', function(data){ 
		var modal = $('#taskModal');
		modal.html(data);
		modal.modal({show:true,backdrop:false}).on("hidden", function(){
	    	$(this).remove();
