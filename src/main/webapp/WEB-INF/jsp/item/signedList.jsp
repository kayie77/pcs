	  ...
	  $.subscribe('exportItem', function(event,data) {
		    var selectedIds = jQuery("#"+data.id).jqGrid('getGridParam','selarrrow');
			 ...
			 if (selectedIds!=null && selectedIds.length>0){
				...
				if(filters!=null){
					$.fileDownload('<s:url value="/item/exportSigned.action"/>', { httpMethod : "POST", data: {filters : filters,pageSize:50}});
				}else{
					$.fileDownload('<s:url value="/item/exportSigned.action"/>', { httpMethod : "POST", data: {pageSize:50}});
			    }
		     }
	  });
...
function showFundDetails(rowId) {
	$('#fundDialog').empty().load("<s:url value="/item/fund/fund.action"/>",{itemId:rowId}).dialog('open'); 
}

function fundLink(cellvalue, options, rowObject) {
    return  "<a style=\"font-weight:bold;text-decoration:underline;color:#ff66;\" href=\"javascript:showFundDetails("+rowObject.id+")\">"+formatNumber(String(cellvalue))+"</a>";
}
</script>
<div class="app-panel">
	<div class="title">当前位置：采购计划 &gt;&gt; 项目查询 &gt;&gt; 已签订合同项目</div>
	<div class="toolbar">
...
<div id="itemWrapper">
	...
	<s:url var="listAdmin" action="listAdmin" namespace="/item" />
	<s:url var="listSigned" action="listSigned" namespace="/item" />
	<sjg:grid dataType="json" href="%{listSigned}" pager="true"
