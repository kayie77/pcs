...
<div id="roleUserModal" class="modal fade" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"></div>
...
$('.page-content-area').ace_ajax('loadScripts', scripts, function() {
  jQuery(function($) {
	...
	$(grid_selector).jqGrid({
		 url:'${ctx}/user/listRoleUsers/${roleId}',
		 datatype: 'json',
	...
	$("#btnRemoveUser").click(function(){
		 ...
		 if(ids!=null && ids.length>0){
			 ...
			 bootbox.confirm("确定删除吗？",function(result) {
				if(result) {	
					 var params={'id':'${roleId}','users':ids};
		 		     $.ajax({
		 			  		type : 'post',
		 			  	    dataType : 'json',
		 			  		url : '${ctx}/role/removeUsers',
  ...
  userCallback = function(users) {
		 if (users.length > 0) {
			 var params={'id':'${roleId}','users':users};	
			 $.ajax({
		  				type : 'POST',
		  				url : '${ctx}/role/addUsers',
		  				data :jQuery.param(params,true),
		  				success : function(data) {
		  					bootbox.alert(data.message);
		  					if(data.success==true){
