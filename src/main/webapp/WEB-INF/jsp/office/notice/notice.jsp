...
$('.page-content-area').ace_ajax('loadScripts', scripts, function() {
  jQuery(function($) {
	...
	$(grid_selector).jqGrid({
		 url:'${ctx}/office/notice/list',
		 datatype: 'JSON',
		 colNames:['ID','通知标题', '发布时间','状态','操作'],
		 colModel:[
		   		...
		   		{name:'status',index:'status',width:50,editable: true,stype: "select",formatter:statusFormatter,
		   			searchoptions:{  
                        sopt:["eq"],
                        stype:"select",
                        dataUrl:"${ctx}/office/notice/statusSearch"
                    }
                },
...
function deleteRow(id){
     $.ajax({
	  		type : 'POST',
	  	    dataType : 'JSON',
	  		url : '${ctx}/office/notice/delete?id='+id,
...
function auditRow(id,status){
	$.ajax({
  		type : 'GET',
  	    dataType : 'JSON',
  		url : '${ctx}/office/notice/auditNotice/'+id+"/"+status,
  		success : function(data) {
  			if(data.success==true){
  				bootbox.dialog({
					message: data.message,
