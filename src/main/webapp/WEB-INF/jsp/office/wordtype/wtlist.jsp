...
<script src="${ctx}/assets/js/common.js"></script>
		<script type="text/javascript">
			...
			function selectMonth(wtMonth,a) {
				
				$(a).tab('show');
				
				var wtYear = $("#wtYear").val();
				var postData = {"wtmonth":wtMonth,"wtyear":wtYear};
				$("#wtMonth").val(wtMonth);
	...
	<div class="col-xs-12">
		...
		<ul class="nav nav-tabs"> 
    	 	<c:forEach var="month" items="${monthList}">
		    	<li <c:if test="${month.key == 1}">class="active"</c:if>><a href="###" onclick="selectMonth('${month.key}',this)">${month.value}</a></li> 
    	 	</c:forEach>
	     </ul> 
    	 <div id="monthDiv" style="margin-top:5px;margin-bottom:5px;display:none;">
    	 	<c:forEach var="month" items="${monthList}">
   				<span id="monthSpan${month.key}" onclick="selectMonth('${month.key}')" style="margin-right:10px;cursor:pointer;<c:if test="${month.key == 1}">background-color:red;</c:if>">${month.value}</span>
   			</c:forEach>
    	 </div>
   ...
   <div class="modal-dialog">
	  <div class="modal-content">
    	 <div class="modal-header">
        	...
        	<h3 class="modal-title">文字类型信息</h3>
    	 </div>
  ...
  jQuery(function($) {
	...
	*/
	...
	$(grid_selector).jqGrid({
		 url:'${ctx}/office/wordtype/queryWtList',
		 datatype: 'JSON',
		 colNames:['ID','文字类型','基础分','操作'],
		 colModel:[
...
function createRow(){
	...
	$.get("${ctx}/office/wordtype/edit?wtyear=${wtYear}&wtmonth="+wtmonth+"&"+Math.random(1000) , '', function(data){ 
		var modal = $('#taskModal');
		modal.html(data);
...
function editRow(id){
	...
	var wtmonth = $("#wtMonth").val();
	$.get("${ctx}/office/wordtype/edit?id="+id+"&wtyear=${wtYear}&wtmonth="+wtmonth+"&"+Math.random(1000) , '', function(data){ 
		var modal = $('#taskModal');
		modal.html(data);
		modal.modal({show:true,backdrop:false}).on("hidden", function(){
	    	$(this).remove();
...
function deleteRow(id){
     $.ajax({
	  		type : 'POST',
	  	    dataType : 'JSON',
	  		url : '${ctx}/office/wordtype/delete?id='+id,
