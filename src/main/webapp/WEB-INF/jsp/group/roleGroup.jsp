...
<div class="row">
	<div class="col-xs-12">
		<div class="btn-group">
			<button id="btnAddGroup" type="button" class="btn btn-info btn-blue btn-sm btn-round" data-toggle="modal">
				<i class="fa fa-plus-square"></i>    
				新增
			</button>
			<button id="btnRemoveGroup" type="button" class="btn btn-info btn-blue btn-sm btn-round">
...
<div id="roleGroupModal" class="modal fade" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"></div>
...
$('.page-content-area').ace_ajax('loadScripts', scripts, function() {
  jQuery(function($) {
	...
	$(grid_selector).jqGrid({
		 url:'${ctx}/group/listRoleGroups/${roleId}',
		 datatype: 'json',
		 colNames:['ID','群组名','群组说明'],
		 colModel:[
		   		{name:'id',index:'id', hidden:true,width:20, editable: true},
		   		{name:'groupName',index:'groupName',width:120, editable: true},
		   		{name:'groupDesc',index:'groupDesc',width:300, sortable:false, editable: true}
		 ],
		 jsonReader : {
	            root: "rows",
	            page: "page",
	...
	$("#btnAddGroup").click(function(){
		$.get("${ctx}/group/select", function(data){ 
			var modal = $('#roleGroupModal');
			modal.html(data);
		 ...
		 if(ids!=null && ids.length>0){
			 ...
			 bootbox.confirm("确定删除吗？",function(result) {
				if(result) {	
					 ...
					 var params={'id':'${roleId}','groups':ids};
		 		     $.ajax({
		 			  		type : 'post',
		 			  	    dataType : 'json',
		 			  		url : '${ctx}/role/removeGroups',
  ...
  groupCallback = function(groups) {
		 if (groups.length > 0) {
			 var params={'id':'${roleId}','groups':groups};	
			 $.ajax({
		  				type : 'POST',
		  				url : '${ctx}/role/addGroups',
		  				data :jQuery.param(params,true),
		  				success : function(data) {
		  					bootbox.alert(data.message); 
		  					if (data.success == true) {
